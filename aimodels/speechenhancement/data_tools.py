...
def audio_files_to_numpy(audio_dir,
    ...
    in a numpy matrix of size (nb_frame,frame_length) for a sliding 
    window of size hop_length_frame"""
...
def numpy_audio_to_matrix_spectrogram(numpy_audio, dim_square_spec, n_fft, hop_length_fft, output=False):
    ...
    if output == True:
        ...
        print('m_mag_db: {}, m_phase: {}'.format(
            np.array(m_mag_db).shape, np.array(m_phase).shape))
